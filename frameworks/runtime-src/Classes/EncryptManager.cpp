...
public:
...
#else
    std::string getStringFromFile(std::string_view filename) CCCOMPAT_CONST_QUALIFIER override
#endif
    {
    ...
    */
    Data getDataFromFile(std::string_view filename) CCCOMPAT_CONST_QUALIFIER override
    {
        ...
        if (data.getSize() > 0 && encryptManager.parseSignInfo((char*)data.getBytes(), data.getSize(), &info)) {
            ...
            if (info.compressed) {
                auto uncomprData = cryptk::zlib_inflate<yasio::byte_buffer>(cxx17::string_view((const char*)data.getBytes(), size), info.original_size);
